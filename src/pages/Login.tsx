...
export default function Login() {
  ...
  const submit = () => {
    ...
    window.setTimeout(() => {
      ...
      setPhase("denied");
      setDeniedFx(true);
      window.setTimeout(() => setDeniedFx(false), Math.max(uiCfg.deniedFlashMs, uiCfg.deniedShakeMs));
    ...
    const tick = () => {
      ...
      for (let col = 0; col < cols; col++) {
        ...
        if (dropsRef.current[col] > rows && Math.random() > respawnChance) {
          ...
          speedsRef.current[col] = rand(matrixCfg.greenSpeedMin, matrixCfg.greenSpeedMax);
        }
      }
  ...
  const flashRed =
    phase === "denied" && deniedFx ? "opacity-30" : "opacity-0";
      ...
      <div className="absolute inset-0 flex items-center justify-center p-6">
        ...
        >
          ...
          <div className="mt-4 space-y-3 text-sm text-green-200/90">
            ...
            {(phase === "verifying" || phase === "granted" || phase === "denied") && (
